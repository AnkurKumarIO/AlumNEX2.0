...
export default function Dashboard() {
  ...
  // Trigger re-render every minute to update "Join Now" button visibility
  // (activates 5 minutes before scheduled start time)
  useEffect(() => {
                  ...
                  <div key={n.id} style={{ background: !n.read ? '#171f33' : '#131b2e', borderRadius: 14, padding: '1.25rem', border: `1px solid ${!n.read ? 'rgba(195,192,255,0.12)' : 'rgba(70,69,85,0.12)'}`, display: 'flex', gap: 14, alignItems: 'flex-start', transition: 'all 0.2s' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '0.65rem', color: 'rgba(199,196,216,0.4)', fontWeight: 600 }}>
                        {toUtcDate(n.createdAt).toLocaleString('en-US', {  weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </div>
                      {/* Join Now button */}
                      {(n.type === 'slot_booked' || n.type === 'live') && (
                        <div style={{ marginTop: 10 }}>
                          {(() => {
                            ...
                            if ((n.type === 'live' || n.type === 'live_now' || n.type === 'reminder' || n.type === 'meeting_live') && joinRoomId) {
                              return (
                                <Link to={joinUrl}
                            ...
                            if (scheduledTime) {
                              return (
                                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '0.4rem 0.875rem', background: 'rgba(78,222,163,0.08)', border: '1px solid rgba(78,222,163,0.2)', borderRadius: 8, fontSize: '0.75rem', color: '#4edea3', fontWeight: 600 }}>
                                  ...
                                  {toUtcDate(scheduledTime).toLocaleString('en-US', {  weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </div>
                              );
                            }
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0b1326', color: '#dae2fd', fontFamily: 'Inter, sans-serif' }}>
      ...
      <aside style={{ width: 256, minHeight: '100vh', position: 'fixed', left: sidebarOpen ? 0 : -256, top: 0, background: '#131b2e', display: 'flex', flexDirection: 'column', padding: '1.5rem', zIndex: 50, transition: 'left 0.3s ease' }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          {/* Subtitle */}
          <div style={{ fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.18em', color: '#c7c4d8', marginTop: 10, textAlign: 'center', width: '100%' }}>Student Portal</div>
        </div>
                        ...
                        <div key={n.id} style={{ padding: '0.875rem 1.25rem', borderBottom: '1px solid rgba(70,69,85,0.1)', display: 'flex', gap: 12, alignItems: 'flex-start', background: !n.read ? 'rgba(195,192,255,0.04)' : 'transparent' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: '0.62rem', color: 'rgba(199,196,216,0.4)', marginTop: 4 }}>
                              {toUtcDate(n.createdAt).toLocaleString('en-US', {  month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </div>
                            {/* Join Now — instant meet or reminder */}
                            {(n.type === 'live' || n.type === 'live_now' || n.type === 'reminder' || n.type === 'meeting_live') && (() => {
                              const req = myRequests.find(r => r.id === n.requestId);
                            ...
                            const joinUrl = `/interview/${n.requestId || req?.id || joinRoomId}?name=${encodeURIComponent(user?.name || 'Student')}`;
                              ...
                              <Link to={joinUrl} onClick={() => setShowNotifs(false)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 8, padding: '0.35rem 0.875rem', background: 'linear-gradient(135deg,#00a572,#4edea3)', color: '#003d29', borderRadius: 8, fontSize: '0.7rem', fontWeight: 700, textDecoration: 'none' }}>
                                ...
                                <div style={{ marginTop: 6, fontSize: '0.68rem', color: '#4edea3', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                                  ...
                                  {scheduledTime ? toUtcDate(scheduledTime).toLocaleString('en-US', {  weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Scheduled'}
                                </div>
                              );
                            })()}
                          </div>
