...
function BookButton({ alumni, studentName, userId, onBook, passedRequests }) {
  ...
  if (status === 'slot_booked') {
    ...
    const formatted = new Date(existing.scheduledTime).toLocaleString('en-US', { timeZone: 'Asia/Kolkata', weekday: 'short', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    return (
          ...
          <div key={a.id || a.name} style={{ background: '#131b2e', borderRadius: 16, padding: '1.5rem', border: '1px solid transparent', transition: 'all 0.3s' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: a.photoPreview ? 'transparent' : 'linear-gradient(135deg,#222a3d,#2d3449)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', fontWeight: 700, color: '#c3c0ff', flexShrink: 0, border: '1px solid rgba(195,192,255,0.1)' }}>
                  {a.photoPreview ? (
                    <img src={a.photoPreview} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  ) : (
                    a.name[0]
                  )}
                </div>
